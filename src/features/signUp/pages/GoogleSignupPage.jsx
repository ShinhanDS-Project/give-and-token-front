...
const GoogleSignupPage = () => {
    ...
    useEffect(() => {
        const fetchGoogleSignupInfo = async () => {
            ...
            try {
                const response = await fetch("http://localhost:8090/api/auth/social-info", {
                    method: "GET",
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errorText || "구글 회원가입 정보를 불러오지 못했습니다.");
                }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await submitSignup("user", payload, formData.profileImage, token);
            if (response.ok) {
                alert("회원가입이 완료되었습니다.");
                navigate("/login");
            } else {
    ...
    if (loadingSocialInfo) {
        return <div>구글 회원가입 정보를 불러오는 중...</div>;
    }
