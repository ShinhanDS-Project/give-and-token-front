...
function resolveDonorDisplayName(donor) {
    if (!donor || typeof donor !== "object") return "기부자";
    if (donor?.isAnonymous === true || donor?.anonymous === true) return "익명";
...
function canAccessDonateAction() {
    ...
    if (!localStorageToken && !hasCookieToken) return false;

    // 소셜 로그인은 쿠키 기반으로만 들어오는 경우를 사용자로 간주한다.
    if (!localStorageToken && hasCookieToken) return true;

    // 로컬 스토리지 토큰이 있으면 role 체크를 강제한다.
    const role = String(window.localStorage.getItem("userRole") || "").toLowerCase();
    return role === "user";
}

function toDetailCampaignModel(data, id) {
    const representativeImage = normalizeImagePath(
    ...
    return {
        ...
        daysLeft: Number.isFinite(Number(data?.daysLeft))
        ...
        recentDonors: Array.isArray(data?.recentDonors)
            ? data.recentDonors.map((donor) => ({
                  ...
                  amount: Number.isFinite(Number(donor?.amount))
                      ? Number(donor.amount)
                      : 0,
                  time: donor?.time || "-",
              }))
        ...
        organization: {
            ...
            image: normalizeImagePath(
                data?.foundation?.profilePath || representativeImage,
            ),
...
export default function CampaignDetail() {
        ...
        async function loadCampaignDetail() {
            try {
                ...
                const campaignResponse = await fetch(
                    `${API_BASE_URL}/api/foundation/campaigns/${id}/detail`,
                );

                if (!campaignResponse.ok)
                    throw new Error(
                        `detail request failed: ${campaignResponse.status}`,
                    );
                const campaignData = await campaignResponse.json();
                if (!ignore)
                    setCampaign(toDetailCampaignModel(campaignData, id));
            } catch {
        ...
        async function loadFinalReport() {
            try {
                ...
                const response = await fetch(
                    `${API_BASE_URL}/api/v1/final-reports/campaign/${id}`,
                );
                if (!response.ok) {
    ...
    const safeRecentDonors = Array.isArray(campaign.recentDonors)
        ? campaign.recentDonors
        : [];
    const safeDocs = Array.isArray(campaign.documents)
        ? campaign.documents
        : [];
    const canAccessDonate = canAccessDonateAction();
                        ...
                        <div className="mb-12 grid grid-cols-4 gap-4">
                            ...
                            <div className="hidden flex-col gap-4 md:flex">
                                {safeImages.slice(1, 4).map((img, idx) => (
                                    <div
                                        key={idx}
                                        className="flex-1 overflow-hidden rounded-[1rem] shadow-md"
                                    >
                                        <img
                                            src={img}
                        ...
                        <div className="mb-10 flex gap-10 border-b border-line">
                            {[
                                ...
                                <button
                                    ...
                                    className={`relative pb-4 text-[1rem] font-bold uppercase tracking-widest transition-all ${
                                        activeTab === tab
                                            ? "text-primary"
                        ...
                        <div className="prose prose-stone max-w-none">
                            {activeTab === "about" && (
                                <div className="space-y-8">
                                    <div className="pt-1">
                                        <h3 className="mb-4 text-[1.7rem] font-bold text-ink">
                                            상세 설명
                                        </h3>
                                    ...
                                    <div>
                                        <h3 className="mb-4 text-[1.7rem] font-display font-bold text-ink">
                                            캠페인 일정 및 예산
                                        </h3>
                                        <div className="grid gap-5 lg:grid-cols-2">
                                            <div className="overflow-hidden rounded-[1.5rem] border border-line bg-white">
                                                <div className="border-b border-line bg-stone-50/80 px-6 py-4 text-sm font-bold uppercase tracking-widest text-stone-400">
                                                    모집 및 사업 기간
                                                </div>
                                                <div className="px-6">
                                                    {[
                                                        ...
                                                        (
                                                            ...
                                                            >
                                                                <span className="text-[1rem] font-medium text-stone-500">
                                                                    {label}
                                                                </span>
                                            ...
                                            <div className="rounded-[1.5rem] border border-line bg-white p-6">
                                                <div className="mb-3 text-sm font-bold uppercase tracking-widest text-stone-400">
                                                    목표 금액
                                                </div>
                                                <div className="mb-2 text-3xl font-display font-bold text-primary">
                                                    {formatWon(campaign.goal)}
                                                </div>
                                                <p className="text-s leading-relaxed text-stone-400">
                                                    현재 캠페인에 설정된 목표
                                                    예산 기준 금액입니다.
                                                </p>
                                            </div>
                                        ...
                                        {safeDocs.length > 0 ? (
                                            <div className="grid gap-4 sm:grid-cols-2">
                                                {safeDocs.map((doc, index) => (
                                                    <div
                                                        key={`${doc.name}-${index}`}
                                                        className="group flex items-center rounded-3xl border border-line bg-white p-6 transition-colors hover:border-primary/30"
                                                    >
                            ...
                            {activeTab === "beneficiary" && (
                                <div className="grid gap-8 md:grid-cols-2 md:items-start">
                                    <div className="space-y-6 rounded-[40px] border border-line bg-white p-8">
                                        <h4 className="mb-4 text-2xl font-display font-bold text-ink">
                                            수혜자 정보
                                        </h4>
                                        <div>
                    ...
                    <div className="lg:col-span-4">
                        <div className="sticky top-32 space-y-8">
                            <div className="rounded-[1.5rem] border-2 border-line bg-white p-10 shadow-m shadow-stone-200/50">
                                ...
                                {isClosedCampaign ? (
                                    ...
                                    <Link
                                        ...
                                        className="mb-4 flex w-full items-center justify-center rounded-full bg-primary py-4 text-base font-bold text-white shadow-m shadow-primary/20 transition-all hover:bg-primary/90"
                                    >
                                        지금 기부하기
                                    </Link>
                                )}
                            ...
                            <div className="rounded-[1.5rem] border-2 border-line bg-white p-10">
                                <h4 className="mb-8 text-2xl font-display font-bold text-ink">
                                    최근 기부자
                                </h4>
                            ...
                            <div className="rounded-[1.5rem] border-2 border-line bg-white p-10">
                                <h4 className="mb-6 text-2xl font-display font-bold text-ink">
                                    기부단체 정보
                                </h4>
                                <FoundationProfileCard
                                    foundation={{
                                        foundationNo:
                                            campaign.organization?.foundationNo,
                                        foundationName:
